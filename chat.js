...
const API_URL = 'https://api.a4f.co/v1/chat/completions'; // Fixed endpoint
...
// Call A4F API - COMPLETELY FIXED VERSION
async function callA4FAPI(message, model) {
    ...
    try {
        ...
        if (!data.choices[0].message || !data.choices[0].message.content) {
            console.error('❌ Invalid message content in response:', data.choices[0]);
            throw new Error('Invalid response structure: no message content');
        }
...
export async function testAPIConnection() {
    const testMessage = "Hello, please respond with 'API is working' if you can see this message.";
    const model = getCurrentModel();
